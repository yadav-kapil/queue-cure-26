...
const HistoryDoc = () => {
  ...
  // Retrieve all completed or skipped consultations across all ended sessions
  const allHistoricalConsultations = useMemo(() => {
  ...
  return (
    <>
      {loading && <Loading message="Loading consultation history..." />}
      {errorMsg && (
        <ErrorDialog
        ...
        <div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-end">
          <div>
            ...
            <p className="mt-2 max-w-2xl text-sm leading-6 text-[#6b7280]">
              Review all patient consultation logs completed during your ended sessions.
            </p>
          </div>
          ...
          <div className="relative">
            <button
              type="button"
              disabled={!allHistoricalConsultations.length}
              onClick={() => {
                ...
                <div className="absolute right-0 mt-2 w-52 rounded-2xl border border-[#e5eaf4] bg-white p-2 shadow-[0_12px_30px_rgba(15,23,42,0.10)] z-50">
                  ...
                  <div className="space-y-0.5">
                    <button
                      type="button"
                      onClick={() => {
                        exportHistory(history, 'doctor', 'csv')
                    ...
                    <button
                      type="button"
                      onClick={() => {
                        exportHistory(history, 'doctor', 'pdf')
                        setShowExportDropdown(false)
                      }}
                      className="w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-bold text-slate-700 hover:bg-[#f8fbff] transition"
                    >
        ...
        <div className="grid gap-5 xl:grid-cols-[minmax(0,1fr)_360px]">
          ...
          <article className="rounded-[24px] border border-[#e5eaf4] bg-white p-4 sm:p-5 shadow-[0_8px_30px_rgba(15,23,42,0.06)] h-[550px] md:h-[680px] flex flex-col">
            ...
            {!loading && history.length === 0 ? (
              <div className="flex-1 flex flex-col justify-center items-center px-5 py-14 text-center border border-dashed border-[#d7e1f0] rounded-[20px] bg-[#f8fbff]">
                ...
                <p className="mt-2 text-sm text-[#6b7280] max-w-sm mx-auto">
                  Historical consultations will display here once you complete sessions.
                </p>
              </div>
          ...
          <aside className="space-y-5">
            <article className="rounded-[24px] border border-[#e5eaf4] bg-white p-5 shadow-[0_8px_30px_rgba(15,23,42,0.06)]">
              ...
              {/* Today's Report */}
              <div className="mb-4">
                <p className="text-xs font-bold uppercase tracking-wider text-[#2459ff] mb-2 font-black">Today&apos;s Report</p>
                <div className="grid gap-2">
                  <HistoryMetric label="Completed Sessions" value={`${stats.sessionsToday} Sessions`} />
                  <HistoryMetric label="Patients Attended" value={`${stats.patientsToday} Patients`} />
                </div>
              </div>

              {/* Total Report */}
              <div>
                <p className="text-xs font-bold uppercase tracking-wider text-slate-400 mb-2 font-black">Total Report</p>
                <div className="grid gap-2">
                  <HistoryMetric label="Completed Till Now" value={`${stats.sessionsTotal} Sessions`} />
                  <HistoryMetric label="Total Patients Attended" value={`${stats.patientsTotal} Patients`} />
                </div>
              </div>
            </article>
