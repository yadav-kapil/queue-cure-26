...
const ManageDocPopover = ({ setIsPopoverOpen, isInline = false }) => {
  ...
  const handleHireClick = async (doctorId) => {
    if (hasPendingSent) {
      setShowReplaceConfirm(doctorId);
    } else {
      setLocalError("");
      try {
  ...
  const handleActionClick = async (reqId, action) => {
    if (action === "accept" && hasHired) {
      setLocalError("Please remove your current doctor first before accepting a new one.");
      return;
    }
    setLocalError("");
    try {
      ...
      <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/40 backdrop-blur-sm p-4">
        <div className="bg-white rounded-3xl w-full max-w-xl max-h-[90vh] overflow-y-auto shadow-2xl flex flex-col">
          <div className="flex items-center justify-between p-5 border-b border-slate-100 sticky top-0 bg-white/95 backdrop-blur z-10">
            <h2 className="text-lg font-extrabold text-[#07122f]">Manage Doctor</h2>
            <button
              ...
              {activeTab === 'search' && (
                <div className="space-y-4">
                  ...
                  {searchResults.length > 0 && !searchLoading && (
                    <div className="space-y-2">
                      {searchResults.map((doc) => (
                        <div key={doc.id || doc._id} className="flex items-center gap-3 p-2.5 rounded-xl border border-slate-100 bg-white hover:bg-slate-50 transition">
                          <span className="grid size-8 place-items-center rounded-lg bg-slate-100 text-slate-400">
                            <FiActivity className="text-sm" />
                          </span>
                          <div className="flex-1 min-w-0">
                            <p className="text-xs font-bold text-[#07122f] truncate">{doc.fullName}</p>
                            <p className="text-[9px] font-semibold text-slate-400">{doc.clinicName || `@${doc.username}`}</p>
                          </div>
                          <button
                            type="button"
                            onClick={() => handleHireClick(doc.id || doc._id)}
...
export default ManageDocPopover;
